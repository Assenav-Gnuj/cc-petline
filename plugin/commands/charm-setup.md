---
name: charm-setup
description: Wire the Charm statusline + cc-petline together — point Claude Code's statusLine at the cc-petline wrapper so the statusline renders AND the pet gets live context%/cost.
---

This finishes the "Charm CC" integration: it repoints `statusLine` in
`~/.claude/settings.json` to **`cc-petline statusline`**, a wrapper that:
...
2. extracts **context% + session cost** into `~/.cc-petline/context`, which the pet's
...
1. Confirm the bundled binary exists: `${CLAUDE_PLUGIN_ROOT}/bin/cc-petline.exe`.
...
2. Run the setup script (it backs up settings.json and remembers the previous
   ...
   ```
   node "${CLAUDE_PLUGIN_ROOT}/scripts/setup-statusline.js" "${CLAUDE_PLUGIN_ROOT}/bin/cc-petline.exe"
   ```

3. Tell the user to start the pet pane (Tabby profile **cc-petline**, or
   `"${CLAUDE_PLUGIN_ROOT}/bin/cc-petline.exe" watch` from the crate root) and that the
...
- `CC_PETLINE_ROWS` — fox height in terminal rows (width = rows×2). Default `6`.
- `CC_PETLINE_GAP` — spaces between the statusline text and the fox. Default `2`.
- `CC_PETLINE_WIDTH` — pin the column inside this total width (set to your terminal's
...
- `CC_PETLINE_ASSETS` — absolute path to the `assets/` dir if the fox doesn't appear
  (the statusLine command's cwd is the project, not the crate).

Example statusLine command with sizing:
`"<plugin>/bin/cc-petline.exe" statusline` and set `CC_PETLINE_WIDTH=120` in env.
...
- **Revert:** `node "${CLAUDE_PLUGIN_ROOT}/scripts/setup-statusline.js" --revert`
  (restores the previous statusLine; backup also at `settings.json.bak-cc-petline`).
